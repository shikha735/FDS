...
/*if(!loggedin())
 {
	echo "Not logged in";*/
	session_start();
    if(isset($_POST['sso_password'])&&isset($_POST['sso_password_again']))
	{
		echo  "All fields set";
		$employee_id = trim($_POST['employee_id']);
	    $employee_id = $_SESSION['employee_id'];
		$sso_password = trim($_POST['sso_password']);
		$sso_password_again = trim($_POST['sso_password_again']);
		if(!empty($sso_password)&&!empty($sso_password_again))
		{
			echo "Not Empty";
			if(strlen($sso_password)>20)
			{
			...
			else
			{
				...
				else
				{
					...
					if($query_num_rows>=1)
					{
				 		$query1 = "UPDATE bankers SET sso_password='.$password_hash.' WHERE employee_id=".$employee_id;
							...
							<div class="row">
								<div class="col-xs-12">
									...
									<div id="login-box-inner" class="with-heading">
										...
										<form role="form" action="forgot-password2.php"  method="POST">
										...
										<div class="input-group">
											...
											<input name="sso_password" type="password" class="form-control" placeholder="Enter new Password">
										</div>
										<div class="input-group">
											<span class="input-group-addon"><i class="fa fa-unlock-alt"></i></span>
											<input name="sso_password_again" type="password" class="form-control" placeholder="Re-enter new Password">
										</div>
